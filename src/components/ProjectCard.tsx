  ...
  return (
    <Link href={`/project/${id}`} className="group block h-full">
      <div className="flex flex-col h-full bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:border-brand/10 transition-all duration-300 card-hover">
        {/* Cover image */}
        <div className="relative h-44 sm:h-52 w-full overflow-hidden bg-gray-100 flex-shrink-0">
          {coverImage ? (
            <Image
              src={coverImage}
              alt={name}
        ...
        <div className="flex flex-col flex-1 p-4 sm:p-5">
          ...
          <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
            <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full flex-shrink-0 ${color.bg} ${color.text}`}>
              {category}
            </span>
          ...
          {/* Name */}
          <h3 className="font-bold text-gray-900 text-sm sm:text-base leading-snug mb-2 group-hover:text-brand transition-colors duration-200">
            {name}
          </h3>

          {/* Description */}
          <p className="text-xs sm:text-sm text-gray-500 leading-relaxed flex-1 line-clamp-2">
            {description}
          </p>

          {/* CTA row */}
          <div className="mt-4 pt-3 border-t border-gray-100 flex items-center gap-1.5 text-brand font-bold text-sm group-hover:gap-3 transition-all duration-200">
            View Details
